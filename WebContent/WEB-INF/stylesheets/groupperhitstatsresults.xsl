		...
		<div class="pagination large-16 medium-16 small-16 row">
			<div class="small-text jump-left large-3 medium-3 small-4 columns">
				...
				<select class="show-select meta-small">
					<xsl:attribute name="onchange">
						<xsl:text>Whitelab.explore.statistics.update('hits',{number : $(this).val(), sort : '</xsl:text><xsl:value-of select="$sortBy" /><xsl:text>'})</xsl:text>
					</xsl:attribute>
					<xsl:choose>
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<li>
								<a>
									<xsl:attribute name="onclick">
										<xsl:text>Whitelab.explore.statistics.update('hits',{first:0, number:</xsl:text><xsl:value-of select="$resultsPerPage"/><xsl:text>, sort : '</xsl:text><xsl:value-of select="$sortBy" /><xsl:text>'})</xsl:text>
									</xsl:attribute>
									<xsl:value-of select="'&lt;&lt;'" />
								</a>
							</li>
			...
			<div class="small-text large-3 medium-3 small-4 columns">
				...
				<button class="small go">
					<xsl:attribute name="onclick">
						<xsl:text>Whitelab.explore.statistics.goToPage('hits',this,</xsl:text><xsl:value-of select="$resultsPerPage"/><xsl:text>,'</xsl:text><xsl:value-of select="$sortBy" /><xsl:text>')</xsl:text>
					</xsl:attribute>
