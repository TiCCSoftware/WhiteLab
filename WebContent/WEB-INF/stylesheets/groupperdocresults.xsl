								...
								<th class="tbl_groupname">
									<a>
										<xsl:attribute name="href">
											...
											<xsl:text>&amp;view=16&amp;group=</xsl:text>
									...
									<td>
										<div class="progress" data-toggle="collapse">
											...
											<xsl:attribute name="onclick">
												<xsl:text>Whitelab.search.result.toggleDocGroupContent('.</xsl:text>
											...
											<div class="inline-concordance">
												<button class="btn btn-link">
													<xsl:attribute name="onclick">
														...
														<xsl:value-of select="$gr" />
														<xsl:text>',true); document.location.href = '/whitelab/search/results?query='+cql+'&amp;view=2&amp;from=4';</xsl:text>
													</xsl:attribute>
												...
												<button class="btn btn-link nolink">
													<xsl:attribute name="onclick">
														<xsl:text>Whitelab.search.result.docGroupContent('#</xsl:text>
														<xsl:value-of select="$rowId" />
														<xsl:text>','</xsl:text>
														<xsl:value-of select="$gr" />
		...
		<div class="large-16 medium-16 small-16 row">
			<select class="group_bySelect">
				<xsl:attribute name="onchange">
					<xsl:text>document.location.href = '</xsl:text>
					<xsl:value-of select="$query_result_url" />
					<xsl:text>&amp;view=16&amp;group='+$(this).val();</xsl:text>
				</xsl:attribute>
				<option value="">
			...
			<div class="small-text jump-left large-3 medium-3 small-4 columns">
				...
				<select class="show-select meta-small">
					<xsl:attribute name="onchange">
						...
						<xsl:value-of select="$query_result_url" />
						<xsl:text>&amp;view=16&amp;number=&quot;+$(this).val()+&quot;&amp;group=</xsl:text>
						<xsl:value-of select="$group_by_name" />
						<xsl:text>&amp;sort=</xsl:text>
						...
						<xsl:otherwise>
							<li>
								<a>
									<xsl:attribute name="href">
										<xsl:value-of select="$query_result_url" />
										<xsl:text>&amp;view=16&amp;first=0&amp;number=</xsl:text>
						...
						<xsl:otherwise>
							<li>
								<a>
									<xsl:attribute name="href">
										<xsl:value-of select="$query_result_url" />
										<xsl:text>&amp;view=16&amp;first=</xsl:text>
			...
			<div class="small-text large-3 medium-3 small-4 columns">
				...
				<button class="small go">
					<xsl:attribute name="onclick">
						...
						<xsl:text>document.location.href='</xsl:text>
						<xsl:value-of select="$query_result_url" />
						<xsl:text>&amp;view=16&amp;first='+value+'&amp;number=</xsl:text>
						<xsl:value-of select="$resultsPerPage"/>
						<xsl:text>&amp;group=</xsl:text>
						<xsl:value-of select="$group_by_name" />
						<xsl:text>&amp;sort=</xsl:text>
