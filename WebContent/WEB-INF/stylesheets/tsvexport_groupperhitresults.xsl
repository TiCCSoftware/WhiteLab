...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="hitGroups">
		<xsl:if test="$include_header = 'true'">
			...
			<xsl:value-of select="$layout" /><xsl:text>&#x9;</xsl:text><xsl:value-of select="$grouped_per_hit" /><xsl:text>&#xa;</xsl:text>
			<xsl:value-of select="$grouped_header" /><xsl:text>&#x9;</xsl:text><xsl:value-of select="../summary/searchParam/group" /><xsl:text>&#xa;&#xa;</xsl:text>
			<xsl:value-of select="../summary/searchParam/group" /><xsl:text>&#x9;</xsl:text><xsl:value-of select="$hits" /><xsl:text>&#xa;</xsl:text>
		</xsl:if>
