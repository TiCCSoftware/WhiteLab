...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="docGroups"><xsl:text>Query&#x9;</xsl:text><xsl:value-of select="../summary/searchParam/patt" /><xsl:text>&#xa;</xsl:text>
		<xsl:choose>
			<xsl:when test="$lang = 'en'">
				<xsl:text>Duration&#x9;</xsl:text><xsl:value-of select="../summary/searchTime" /><xsl:text> ms</xsl:text><xsl:text>&#xa;</xsl:text>
				<xsl:text>Layout&#x9;Group per document</xsl:text><xsl:text>&#xa;&#xa;</xsl:text>
				<xsl:text>Grouped by&#x9;</xsl:text><xsl:value-of select="$groupBy_name" /><xsl:text>&#xa;&#xa;</xsl:text>
				<xsl:value-of select="$groupBy_name" /><xsl:text>&#x9;Documents</xsl:text><xsl:text>&#xa;</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>Duur&#x9;</xsl:text><xsl:value-of select="../summary/searchTime" /><xsl:text> ms</xsl:text><xsl:text>&#xa;</xsl:text>
				<xsl:text>Layout&#x9;Groepeer per document</xsl:text><xsl:text>&#xa;&#xa;</xsl:text>
				<xsl:text>Gegroepeerd per&#x9;</xsl:text><xsl:value-of select="$groupBy_name" /><xsl:text>&#xa;&#xa;</xsl:text>
				<xsl:value-of select="$groupBy_name" /><xsl:text>&#x9;Documenten</xsl:text><xsl:text>&#xa;</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
